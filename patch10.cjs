                                 ...
                                 <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', marginTop: '0.5rem' }}>
                                   {payrollViewMode === 'outstanding' ? (
                                     <>
                                       ...
                                       {perms.root && (
                                         <button className="btn btn-danger" onClick={() => handleDeletePayrollRecord(wId)} style={{ padding: '0.6rem' }}>
                                           <Trash2 size={16} />
                                         </button>
                                       )}
                                     </>
...
if(content.includes("desktop-only") || content.includes("MOBILE CARD VIEW")) {
...
if(content === originalContent) {
    console.log("Failed to match target. Creating a fallback replace using substrings.");
    process.exit(1);
} else {
