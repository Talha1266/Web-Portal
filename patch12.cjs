...
const targetStart = 1805;
const targetEnd = 1901;

// verify line contents just in case
if (!lines[targetStart - 1].includes("return (")) {
    console.error("Line " + targetStart + " does not contain 'return ('. Contains: " + lines[targetStart - 1]);
    process.exit(1);
}
if (!lines[targetEnd - 1].includes(");")) {
    console.error("Line " + targetEnd + " does not contain ');'. Contains: " + lines[targetEnd - 1]);
    process.exit(1);
}

// splice the array
lines.splice(targetStart - 1, targetEnd - targetStart + 1, replacement);

fs.writeFileSync('src/pages/UserDashboard.jsx', lines.join('\n'));
console.log("Successfully replaced table rendering with dual layout by exact line array slice!");
